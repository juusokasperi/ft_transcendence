...
export function signAccessToken(payload: JWTPayload): string {
  ...
  return jwt.sign(tokenPayload, SECRET, { expiresIn: JWT_ACCESS_TTL });
}
...
export function signTwoFactorToken(payload: JWTPayload): string {
  ...
  return jwt.sign(tokenPayload, SECRET, { expiresIn: JWT_2FA_TTL });
}
...
export function verifyTwoFactorToken(token: string): TwoFactorTokenPayload {
  const decoded = jwt.verify(token, SECRET) as TwoFactorTokenPayload;
