...
export async function gamesRoutes(app: FastifyInstance) {
  ...
  async (req: FastifyRequest, res: FastifyReply) => {
    const transaction = db.transaction(() => {
      ...
      const team1Points = calculateEloChange(team1AvgElo, team2AvgElo, team1Result, tournamentStage);
      const team2Points = calculateEloChange(team2AvgElo, team1AvgElo, team2Result, tournamentStage);

      const gameId = addGame(team1Score, team2Score, team1Players,
        team2Players, team1Points, team2Points, tournamentId, tournamentStage);
      if (!gameId)
