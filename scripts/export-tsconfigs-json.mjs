...
const SCRIPT = 'export-tsconfigs-json';
const ROOT = path.resolve(__dirname, '..');
const SEARCH_ROOT = path.resolve(ROOT, process.argv[2] ?? ROOT);
const START = Date.now();
const OUT_DIR = path.join(ROOT, 'scripts', 'output');
const OUT_FILE = path.join(OUT_DIR, 'tsconfig-json.txt');

const IGNORE_DIRS = new Set(['node_modules','.git','.turbo','.cache','dist','build','coverage','tmp','temp']);
const TARGET_NAMES = new Set(['tsconfig.json','tsconfig.base.json','tsconfig.app.json','tsconfig.node.json','tsconfig.build.json']);
...
async function *walk(dir) {
  ...
  for (const e of entries) {
    ...
    } else if (e.isFile() && TARGET_NAMES.has(e.name)) {
      yield full;
    }
  }
}
...
async function main() {
  ...
  console.log(`${SCRIPT}:\n  found: ${files.length} tsconfig files\n  wrote: ${rel(OUT_FILE)}\n  time: ${secs}s`);
}
