...
const TEXT_EXTS = new Set(['.md','.txt','.ts','.tsx','.js','.jsx','.json','.yml','.yaml','.css','.scss','.html','.c','.cpp','.h','.hpp','.rs']);
...
const sanitizeOutName = (relPath) => relPath.replace(/^[./\\]+/, '').replace(/[\\/]+/g, '__') + '.txt';
...
async function exportFolder(relDir) {
  ...
  let stat;
  try { stat = await fsp.stat(absDir); } catch { stat = null; }
  ...
  if (!stat || !stat.isDirectory()) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (s.size > MAX_FILE_BYTES) block = `===== BEGIN ${rel(file)} (SKIPPED: > ${MAX_FILE_BYTES} bytes) =====\n`;
        else if (!isProbablyText(file)) block = `===== BEGIN ${rel(file)} (SKIPPED: non-text) =====\n`;
        else {
