...
define ensure_builder
	...
	@if ! docker buildx inspect $(BUILDER) >/dev/null 2>&1; then \
		...
		docker buildx create --name $(BUILDER) --driver docker-container --driver-opt image=$(BUILDER_IMAGE) --buildkitd-flags '--oci-max-parallelism 4' >/dev/null; \
	fi
...
builder-init:
	...
	@if docker buildx inspect $(BUILDER) >/dev/null 2>&1; then \
			...
			docker buildx create --name $(BUILDER) --driver docker-container --driver-opt image=$(BUILDER_IMAGE) --buildkitd-flags '--oci-max-parallelism 4' --use; \
	fi
